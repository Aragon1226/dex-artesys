        ...
        if (domain === "api.binance.us" && !detectedUsDomain) {
          ...
          if (ws) {
            try {
              ...
              console.debug("Error recycling WS:", e);
            }
            ws = null;
      ...
      if (!detectedUsDomain) {
        ...
        if (ws) {
          try {
            ...
            console.debug("Error on WS fallback:", e);
          }
          ws = null;
        }
...
export const marketService = {
  ...
  getCurrentPrice: (symbol: string): number => {
    ...
    const fallback =
      cachedPrices[s] ||
      FALLBACK_PRICES[s] ||
      (FAKE_SYMBOLS.includes(s) ? generateRandomPrice(s) : 0);
  ...
  get24hTickerStats: async (symbols: string[]): Promise<Record<string, MarketTicker>> => {
    ...
    symbols.forEach((s) => {
      ...
      const defaultPrice = FAKE_SYMBOLS.includes(sym)
        ? generateRandomPrice(sym)
        : FALLBACK_PRICES[sym] || 100;
      ...
      if (controlled.isControlled) {
        ...
        pChange =
          startP > 0 ? parseFloat((((controlled.price - startP) / startP) * 100).toFixed(2)) : 0;
      }
            ...
            if (symbols.includes(item.symbol)) {
              ...
              if (controlled.isControlled) {
                ...
                stats[item.symbol] = {
                  ...
                  priceChangePercent:
                    startP > 0
                      ? parseFloat((((controlled.price - startP) / startP) * 100).toFixed(2))
                      : 0,
                };
              } else {
  ...
  getPrices: async (): Promise<Record<string, number>> => {
    ...
    Object.keys(FALLBACK_PRICES).forEach((s) => {
      const controlled = tokenPriceControl.getControlledPrice(
        s,
        cachedPrices[s] || FALLBACK_PRICES[s],
      );
      if (controlled.isControlled) {
