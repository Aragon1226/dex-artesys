...
export const SAMPLE_TOKENS_LIST = [
  ...
  { symbol: "BOT", name: "Bot Trading Engine", category: "Main", defaultPrice: 123.5 },
  ...
  { symbol: "BEX", name: "Block Exchange L2", category: "Main", defaultPrice: 128.3 },
  ...
  { symbol: "AUR", name: "Aurora Synth", category: "Main", defaultPrice: 12.5 },
  { symbol: "VTX", name: "Vortex Protocol", category: "Main", defaultPrice: 4.8 },
  { symbol: "NEX", name: "Nexus Chain", category: "Main", defaultPrice: 0.95 },
  { symbol: "GLX", name: "Galaxya Network", category: "Main", defaultPrice: 22.4 },
  { symbol: "PHX", name: "Phoenix Token", category: "Main", defaultPrice: 1.25 },
  { symbol: "ZEL", name: "Zeta Labs", category: "Main", defaultPrice: 0.45 },
  { symbol: "CRY", name: "Crypto Alpha", category: "Main", defaultPrice: 8.5 },
  ...
  { symbol: "CPX", name: "Artesys Inc", category: "Stocks & Commodities", defaultPrice: 285.5 },
  { symbol: "ARTS", name: "Artesys Corp", category: "Stocks & Commodities", defaultPrice: 164.2 },
  { symbol: "AXG", name: "AeroX Global", category: "Stocks & Commodities", defaultPrice: 215.8 },
  { symbol: "BGNX", name: "BioGenix Labs", category: "Stocks & Commodities", defaultPrice: 92.4 },
  { symbol: "VOLT", name: "Volt Motors", category: "Stocks & Commodities", defaultPrice: 148.6 },
  {
    symbol: "QCORE",
    name: "QuantumCore Tech",
    category: "Stocks & Commodities",
    defaultPrice: 312.0,
  },
  {
    symbol: "CYBR",
  ...
  { symbol: "OMNI", name: "OmniMart Group", category: "Stocks & Commodities", defaultPrice: 230.1 },
  { symbol: "CINE", name: "CineVerse Media", category: "Stocks & Commodities", defaultPrice: 78.9 },
  {
    symbol: "TFRA",
    name: "Terra Infra Group",
    category: "Stocks & Commodities",
    defaultPrice: 112.5,
  },
];
      ...
      if (status === "SUBSCRIBED") {
        ...
        loadTokenPricesFromDB().then(() => {
          if (
            ...
            Object.keys(manualOverridesMap).length === 0
          ) {
            broadcastChannel
              .send({
                type: "broadcast",
...
export const tokenPriceControl = {
    ...
    if (schedule && schedule.isActive) {
      ...
      if (schedule.returnInitiatedAt) {
        ...
        if (elapsedSinceReturn >= returnDurationMs) {
          ...
          const fromPrice =
            schedule.returnStartPrice !== undefined
              ? schedule.returnStartPrice
              : schedule.targetPrice;
          const returnBasePrice =
            schedule.returnBasePrice !== undefined
              ? schedule.returnBasePrice
              : tokenMeta?.defaultPrice || schedule.startPrice;
        ...
        const rawPrice =
          schedule.startPrice + (schedule.targetPrice - schedule.startPrice) * progressRatio;
  ...
  setSchedule: (params: {
    ...
    const durText =
      params.durationHours >= 24
        ? `${(params.durationHours / 24).toFixed(1)} day(s)`
        : `${params.durationHours} hour(s)`;
  ...
  startReturnToBase: (
    ...
    const durationHours =
      customDurationHours || schedule.returnDurationHours || 1 + Math.random() * 3;
  ...
  cancelReturnToBase: (symbol: string, adminEmail?: string) => {
    ...
    if (schedule && schedule.returnInitiatedAt) {
      ...
      logAction(
        cleanSym,
        "CANCEL_RETURN_TO_BASE",
        `Paused/Cancelled return to base; holding at target price for ${cleanSym}`,
        adminEmail || "admin",
      );
  ...
  cancelSchedule: (symbol: string, adminEmail?: string) => {
    ...
    if (schedulesMap[cleanSym]) {
      ...
      logAction(
        cleanSym,
        "CANCEL_SCHEDULE",
        `Cancelled active trend schedule for ${cleanSym}`,
        adminEmail || "admin",
      );
  ...
  resetToken: (symbol: string, adminEmail?: string) => {
    ...
    logAction(
      cleanSym,
      "RESET_DEFAULT",
      `Reset ${cleanSym} to default market calculation`,
      adminEmail || "admin",
    );
  ...
  resetAllTokens: (adminEmail?: string) => {
    ...
    const lockedTokens = Object.values(schedulesMap).filter(
      (sch) =>
        sch.isActive && sch.endTime > Date.now() && (sch.createdByAdmin || "admin") !== email,
    );
    if (lockedTokens.length > 0) {
      throw new Error(
        `Cannot reset all: ${lockedTokens.map((t) => t.symbol).join(", ")} are locked by other admins.`,
      );
    }
    ...
    logAction(
      "ALL",
      "RESET_ALL",
      "Reset all sample tokens to standard uncontrolled defaults",
      adminEmail || "admin",
    );
