  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      if (fromAsset === "USDT") {
        ...
        await supabase
          .from("profiles")
          .update({ balance: Math.max(0, currentBal - amount) })
          .eq("id", user.id);
      } else {
      ...
      if (toAsset === "USDT") {
        ...
        const { error: profUpdErr } = await supabase
          .from("profiles")
          .update({ balance: (profile?.balance || 0) + destinationAmount })
          .eq("id", user.id);
        ...
        if (existingAsset) {
          const { error: updErr2 } = await supabase
            .from("user_assets")
            .update({ amount: Number(existingAsset.amount || 0) + destinationAmount })
            .eq("user_id", user.id)
            .eq("symbol", toAsset);
          ...
          const { error: insErr } = await supabase
            .from("user_assets")
            .insert({ user_id: user.id, symbol: toAsset, amount: destinationAmount });
  ...
  return (
    <div className="bg-card rounded-3xl p-6 border border-border shadow-sm">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={fetchPrices}
          className={`p-2 rounded-full hover:bg-muted transition-colors ${loading ? "animate-spin" : ""}`}
        >
          ...
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-xs text-muted-foreground">
              ...
              <span className="text-foreground font-bold">
                {userBalance.toFixed(fromAsset === "USDT" ? 2 : 6)} {fromAsset}
              </span>
            </span>
          </div>
      ...
      <div className="mt-6 space-y-4">
        <div className="flex items-center justify-between text-xs px-2">
          ...
          <span className="text-foreground font-medium">
            1 {fromAsset} ≈ {(prices[fromAsset] / (prices[toAsset] || 1)).toFixed(6)} {toAsset}
          </span>
        </div>
