...
const UserHome = () => {
  ...
  useEffect(() => {
    ...
    const shouldReset =
      hash.includes("action=reset_password") ||
      sessionStorage.getItem("open_password_reset") === "true";
    if (shouldReset) {
      ...
      try {
        ...
        if (!error && data && data.length > 0) {
          const matchedRow = data.find(
            (r: any) =>
              (r as any).admin_id === assignedAdminId || (r as any).adminId === assignedAdminId,
          );
          if (matchedRow) {
          ...
          const globalRow = data.find(
            (r: any) => r.email === "global_support_configs@crypxpro.com",
          );
          ...
          const defaultRow =
            data.find((r: any) => r.email !== "global_support_configs@crypxpro.com") || data[0];
          if (defaultRow) {
  ...
  const uploadKYCFile = async (file: File, docType: string): Promise<string | null> => {
    ...
    try {
      ...
      if (error) {
        ...
        console.warn(
          "KYC Bucket issue detected, falling back to base64 encoding to preserve documents.",
        );
        try {
  ...
  if (loading || (!profile && user))
    return <CubeSpinner fullScreen label="Initializing dashboard..." />;
  ...
  const totalBalance =
    spotBalanceValue + (profile?.futures_balance || 0) + (profile?.staked_balance || 0);
    ...
    <>
      ...
      <div className="pb-24 relative min-h-screen bg-background overflow-x-hidden">
        <div className="max-w-7xl mx-auto px-4 pt-2.5 space-y-4">
          ...
          <div
            className="bg-card p-5 rounded-3xl border border-border mt-0.5 shadow-sm relative overflow-hidden group active:scale-[0.99] transition-all cursor-pointer"
                    ...
                    <Pie
                      data={
                        [
                          ...
                          {
                            name: "Futures",
                            value: profile?.futures_balance || 0,
                            color: "#8b5cf6",
                          },
                          ...
                          ? [
                              ...
                              {
                                name: "Earn",
                                value: profile?.staked_balance || 0,
                                color: "#10b981",
                              },
                            ].filter((d) => d.value > 0)
                          : [{ name: "Empty", value: 1, color: "#3f3f46" }]
                      }
                      dataKey="value"
                      nameKey="name"
                      cx="50%"
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {[
              ...
              { symbol: "PRO", cat: "Listed", price: 806.0, change: 11.17 },
              { symbol: "PONKE", cat: "Meme", price: 0.0421, change: 25.16 },
              { symbol: "Gold", cat: "Metal", price: 4539.11, change: -2.5 },
            ].map((item, i) => (
              <div
                key={i}
                className="bg-card p-2.5 rounded-xl border border-border flex flex-col justify-between h-24 shadow-sm"
              >
                <div className="flex justify-between items-start">
                  <div className="min-w-0">
                    <div className="flex items-center gap-1 mb-1">
                      ...
                      <span className="text-[7.5px] text-muted-foreground font-bold uppercase tracking-tight">
                        {item.cat}
                      </span>
                    </div>
                    <div className="space-y-0.5">
                      <div className="font-bold text-foreground text-[13px] font-mono">
                  ...
                  <div
                    className={`w-8 h-4 ${item.change >= 0 ? "text-success/30" : "text-danger/30"}`}
                  >
              ...
              {TOP_SYMBOLS.map((symbol) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => navigate("/app/trade-fi?tab=spot")}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-1">
                          ...
                          <span className="bg-muted text-muted-foreground text-[8px] font-bold px-1 rounded uppercase">
                            Perp
                          </span>
                        </div>
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div
                        className={`w-20 py-2 rounded-lg text-white font-bold text-xs text-center ${isUp ? "bg-success" : "bg-danger"}`}
                      >
                        {isUp ? "+" : ""}
                        {change}%
                      </div>
      ...
      {activeModal === "MENU" && (
        <div
          className="fixed inset-0 z-[70] bg-black/50 backdrop-blur-sm overflow-hidden"
          onClick={() => setActiveModal(null)}
        >
          <div
            className="absolute top-0 left-0 bottom-0 w-[85%] max-w-xs bg-background shadow-2xl flex flex-col overflow-hidden"
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              <div className="grid grid-cols-2 gap-2 mb-4">
                <div className="bg-card p-3 rounded-xl border border-border shadow-sm">
                  ...
                  <p className="font-bold text-foreground text-sm font-mono text-primary">
                ...
                <div className="bg-card p-3 rounded-xl border border-border shadow-sm">
                  ...
                  <p className="font-bold text-foreground text-sm font-mono">
                    ${spotBalanceValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                  </p>
                </div>
              </div>

              <p className="px-2 text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2 mt-4">
                Account
              </p>
              <MenuBtn
                icon={User}
                ...
                onClick={() => setActiveModal("PROFILE_DETAILS")}
              />
              <MenuBtn
                icon={Shield}
              ...
              <p className="px-2 text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2 mt-6">
                General
              </p>
              <MenuBtn
                icon={Settings}
            ...
            <div className="p-4 bg-card border-t border-border">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 p-3 rounded-xl bg-muted text-muted-foreground hover:bg-destructive/10 hover:text-destructive font-bold transition-colors"
              >
          ...
          <div className="bg-card w-full max-w-sm rounded-[32px] max-h-[90vh] flex flex-col shadow-2xl animate-scale-in border border-border">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="flex flex-col items-center mb-8">
                ...
                <span className="px-3 py-1 bg-muted rounded-lg text-xs font-mono text-muted-foreground border border-border">
                  ID: {profile.ftid}
                </span>
              </div>
              ...
              {profile.kyc_status === "VERIFIED" ? (
                ...
                <div className="flex flex-col items-center justify-center py-10 text-center">
                  ...
                  <p className="text-muted-foreground text-sm px-4">
                    Your documents have been submitted and are being reviewed. This usually takes
                    1-3 business days.
                  </p>
                </div>
                  ...
                  <div className="space-y-4 mb-6">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-muted border border-border rounded-xl px-4 py-3 text-sm text-foreground focus:border-primary outline-none"
                      />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-muted-foreground uppercase mb-1.5 block">
                ...
                <>
                  ...
                  <div className="space-y-4 mb-6">
                    <FileUploadField
                      ...
                      required
                      file={kycForm.idFront}
                      onFile={(f) => setKycForm({ ...kycForm, idFront: f })}
                    />
                    <FileUploadField
                      label="Government ID (Back)"
                      file={kycForm.idBack}
                      onFile={(f) => setKycForm({ ...kycForm, idBack: f })}
                    />
                    <FileUploadField
                      label="Selfie Holding ID"
                      required
                      file={kycForm.selfie}
                      onFile={(f) => setKycForm({ ...kycForm, selfie: f })}
                    />
                  </div>

                  {kycUploadProgress && (
                    <div className="mb-4 bg-primary/5 border border-primary/10 rounded-xl p-3 text-center text-sm font-medium text-primary">
                      {kycUploadProgress}
                    </div>
                  )}
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className="flex-1 bg-muted text-foreground font-bold py-3.5 rounded-xl"
                    >
                      Back
                    </button>
                    <button
              ...
              <div className="bg-primary/5 border border-primary/10 rounded-xl p-5 flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">
                    High - Password & Session Encryption Active
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    },
                    {
                      ...
                      label: "Login History",
                      onClick: () =>
                        toast.info("Feature coming soon... Your current active session is secure."),
                    },
                    {
                      icon: Eye,
                      ...
                      onClick: () =>
                        toast.info(
                          "Feature coming soon... Secure sessions are monitored in real-time.",
                        ),
                    },
                  ].map((item) => (
                    <button
            ...
            <div className="flex justify-between items-center p-6 border-b border-border">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  App policies, terms & educational guide
                </p>
              </div>
              <button
                onClick={() => setActiveModal("MENU")}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-[11px] text-muted-foreground">
                      Developer & operator liability exemption
                    </span>
                  </div>
                </div>
                <ChevronRight
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-[11px] text-muted-foreground">
                      Spot, Futures, Earn & KYC explanations
                    </span>
                  </div>
                </div>
                <ChevronRight
          ...
          <div className="bg-card w-full max-w-lg rounded-[32px] max-h-[90vh] flex flex-col shadow-2xl animate-scale-in border border-border">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 text-sm text-muted-foreground leading-relaxed custom-scrollbar">
              <div className="p-4 rounded-2xl bg-warning/10 border border-warning/20 text-warning text-xs leading-relaxed">
                <strong className="text-warning block mb-1">
                  Educational Sector & Demo Purpose:
                </strong>
                Artesys is created exclusively for demo trading, market learning, and software
                evaluation purposes. It does not provide real financial services, live money
                transactions, or financial advice.
              </div>

              <section>
                <h4 className="font-bold text-foreground mb-1">
                  1. Educational Purpose & Non-Financial Intent
                </h4>
                <p>
                  By accessing or using Artesys, you acknowledge that all trading features, spot
                  balances, futures leverage, and yield returns are simulated paper credits designed
                  strictly for learning and educational evaluation.
                </p>
              </section>
              ...
              <section>
                ...
                <p>
                  The development team, software authors, and platform operators hold zero legal
                  liability or responsibility for any user actions, reliance on simulated data, or
                  decisions made after signing up or using this application.
                </p>
              </section>
              ...
              <section>
                ...
                <p>
                  You must be at least 18 years old and capable of agreeing to software evaluation
                  terms. Account data is maintained securely for testing and session state tracking.
                </p>
              </section>

              <section>
                <h4 className="font-bold text-foreground mb-1">
                  4. Trading Features & Yield Simulation
                </h4>
                <p>
                  Artesys provides simulated spot trading, futures leverage practice, and staking
                  yield simulation. No real money deposits or guaranteed financial returns exist.
                </p>
              </section>

              <section>
                <h4 className="font-bold text-foreground mb-1">
                  5. Scamadviser & App Store Transparency
                </h4>
                <p>
                  This app operates with full transparency. It does not engage in real financial
                  solicitation, predatory trading schemes, or unauthorized asset management.
                </p>
              </section>
            </div>
            ...
            <div className="p-6 border-b border-border flex justify-between items-center shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Privacy, Disclaimers & Developer Safeguards
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 text-sm text-muted-foreground leading-relaxed custom-scrollbar">
              <div className="p-4 rounded-2xl bg-destructive/10 border border-destructive/20 text-destructive-foreground space-y-1">
                ...
                <p className="text-xs text-red-300/90 leading-relaxed">
                  By registering or using Artesys, the user explicitly agrees that the software
                  developers, authors, and operators bear NO liability or responsibility after
                  sign-up. The app is not a financial institution or financial advisory service.
                </p>
              </div>
              ...
              <section>
                ...
                <p>
                  This software is built for educational demonstration, user interface testing, and
                  crypto mechanics education. No financial advisory or money management services are
                  offered or implied.
                </p>
              </section>
              ...
              <section>
                ...
                <p>
                  Account credentials (email, display name) are stored strictly for session
                  management and demo profile state. Personal data is never sold to third parties.
                </p>
              </section>
              ...
              <section>
                ...
                <p>
                  Uploaded documents in the identity verification section are handled in a sandbox
                  environment to demonstrate compliance workflows for educational evaluation.
                </p>
              </section>

              <section>
                <h4 className="font-bold text-foreground mb-1">
                  4. Official Inquiries & Data Deletion
                </h4>
                <p>
                  Users may request account deletion or contact platform administration directly at{" "}
                  <span className="text-foreground font-semibold">admin@artesys.com</span>. Official
                  inquiries are handled within 24–48 hours.
                </p>
              </section>
            </div>
          ...
          <div className="bg-card w-full max-w-lg rounded-[32px] max-h-[90vh] flex flex-col shadow-2xl animate-scale-in border border-border">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {[
                {
                  q: "What is Artesys?",
                  a: "Artesys is a cryptocurrency trading platform that offers spot trading, futures trading, staking, and asset management services. Our mission is to make digital asset trading accessible to everyone.",
                },
                {
                  q: "How do I create an account?",
                  a: 'Tap "Sign Up" on the login screen, enter your email and a secure password, then verify your email address. Once verified, you can start using the platform immediately.',
                },
                {
                  q: "What is KYC and why is it required?",
                  a: "KYC (Know Your Customer) is an identity verification process required by financial regulations. Completing KYC unlocks full platform features including higher withdrawal limits and futures trading.",
                },
                {
                  q: "How do I deposit funds?",
                  a: 'Go to the Assets page, tap "Deposit", select your preferred cryptocurrency and network, then send funds to the provided wallet address. Deposits are credited after blockchain confirmation.',
                },
                {
                  q: "How do I withdraw funds?",
                  a: 'Navigate to Assets, tap "Withdraw", select the token and network, enter the destination address and amount, then confirm. Withdrawals are processed after security verification.',
                },
                {
                  q: "What trading pairs are available?",
                  a: "We support major trading pairs including BTC/USDT, ETH/USDT, BNB/USDT, SOL/USDT, XRP/USDT and more. Visit the Spot page for the full list of available pairs.",
                },
                {
                  q: "What is futures trading?",
                  a: "Futures trading allows you to trade with leverage, amplifying potential gains (and losses). You can go long (buy) or short (sell) on supported pairs with leverage up to 100x.",
                },
                {
                  q: "What is staking?",
                  a: "Staking lets you earn passive rewards by locking your tokens for a set period. Visit the Earn page to explore available staking plans and their APY rates.",
                },
                {
                  q: "How do I convert between tokens?",
                  a: "Use the Quick Convert feature on the Spot page. Select the token pair, enter the amount, and confirm the conversion. The exchange rate is calculated in real-time.",
                },
                {
                  q: "Is my account secure?",
                  a: "Yes. We implement industry-standard security measures including encrypted data storage, secure password authentication, and real-time session protection.",
                },
                {
                  q: "How do I contact support?",
                  a: "Go to your Profile menu → Help & Support or email our official desk directly at admin@artesys.com. Support tickets are reviewed within 24–48 hours.",
                },
                {
                  q: "What fees does Artesys charge?",
                  a: "We charge competitive trading fees on spot and futures trades. Deposit fees depend on the blockchain network. Detailed fee information is available on the platform.",
                },
              ].map((faq, i) => (
                <details
                  key={i}
                  className="group bg-muted/50 rounded-xl border border-border overflow-hidden"
                >
          ...
          <div className="bg-card w-full max-w-sm rounded-[32px] p-6 shadow-2xl relative animate-scale-in border border-border">
            ...
            <div className="bg-primary/5 rounded-xl p-6 text-center mb-6 border border-primary/10">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                Our support team is available around the clock. Inquiries sent to our official
                channel are addressed promptly.
              </p>
            ...
            {!supportInfo ||
            (!supportInfo.telegram && !supportInfo.whatsapp && !supportInfo.email) ? (
              <div className="space-y-3 mb-6">
                <a
                  href="mailto:admin@artesys.com"
                  className="block w-full py-3 bg-primary text-primary-foreground font-bold rounded-xl text-center shadow-brand text-xs"
                >
              ...
              <div className="space-y-3 mb-6">
                ...
                {supportInfo.whatsapp && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="block w-full py-3 bg-success text-white font-bold rounded-xl text-center text-xs"
                  >
                    WhatsApp Support
                  </a>
                )}
                <a
                  href={`mailto:${supportInfo.email || "admin@artesys.com"}`}
                  className="block w-full py-3 bg-card border border-border text-foreground font-bold rounded-xl text-center hover:bg-accent text-xs"
                >
...
const MenuBtn = ({
  ...
  <button
    onClick={onClick}
    className="w-full flex items-center gap-3 p-3 bg-card hover:bg-accent rounded-xl transition-colors group border border-transparent hover:border-border"
  >
