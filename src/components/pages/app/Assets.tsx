...
const Assets = () => {
  ...
  const [activeModal, setActiveModal] = useState<
    "deposit" | "withdraw" | "transfer" | "history" | null
  >(null);
    ...
    if (activeModal === "deposit") {
      ...
      if (customAddress) {
        ...
        supabase
          ...
          .eq("network", selectedToken.network)
          .single()
  ...
  const handleTransfer = async () => {
    ...
    if (transferFrom === "Spot" && val > (profile.balance || 0)) {
      toast.error("Insufficient Balance");
      return;
    }
    if (transferFrom === "Futures" && val > (profile.futures_balance || 0)) {
      toast.error("Insufficient Balance");
      return;
    }
    setIsTransferring(true);
    let newSpot = profile.balance || 0,
      newFutures = profile.futures_balance || 0;
  ...
  const filteredTokens = DEPOSIT_OPTIONS.filter(
    (opt) =>
  ...
  const totalAssetValue =
    spotBalance + (profile.futures_balance || 0) + (profile.staked_balance || 0);
        ...
        <div className="bg-card rounded-2xl p-5 mb-4 overflow-hidden relative shadow-lg border border-border">
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-baseline gap-1.5 mb-6">
              ...
              <AnimatedBalance
                value={totalAssetValue.toLocaleString("en-US", {
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { label: "Spot", val: spotBalance },
                { label: "Futures", val: profile.futures_balance },
                { label: "Earn", val: profile.staked_balance },
              ].map((item) => (
                <div
                  key={item.label}
              ...
              <div className="space-y-3">
                {historyLoading ? (
                  ...
                  historyData.map((item) => (
                    <div
                      key={item.id}
                      className="bg-muted/30 border border-border p-3 rounded-xl flex justify-between items-center"
                    >
                      <div className="flex gap-3 items-center">
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center ${historyTab === "deposits" ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}
                        >
                          {historyTab === "deposits" ? (
                            <ArrowDown size={14} />
                          ) : (
                      ...
                      <div className="text-right">
                        <p
                          className={`text-sm font-mono font-bold ${historyTab === "deposits" ? "text-success" : "text-foreground"}`}
                        >
                          {historyTab === "deposits" ? "+" : "-"}
                          {item.amount}
                        </p>
                ...
                <>
                  ...
                  <div className="mb-5">
                    ...
                    <input
                      ...
                      placeholder="0.00"
                      className="w-full bg-muted border border-border rounded-xl px-4 py-2.5 font-mono text-base text-foreground focus:ring-2 focus:ring-primary/10 focus:border-primary/50 outline-none"
                    />
                  </div>
                  ...
                  <div className="bg-warning/10 border border-warning/20 rounded-lg p-3 mb-6 flex gap-2.5">
                    ...
                    <p className="text-[11px] text-warning font-bold leading-tight">
                      ...
                      . Incorrect assets will be lost permanently.
                    </p>
                  </div>

                  <button
                    onClick={handleDepositSubmit}
                    disabled={isSubmitting || !depositAmount || !depositImage}
                ...
                <div className="p-4 border-b border-border">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-muted border border-border rounded-xl pl-9 pr-4 py-2.5 text-xs font-bold text-foreground focus:outline-none focus:border-primary/50 transition-colors"
                      autoFocus
                    />
                  </div>
                </div>
                  ...
                  {filteredTokens.length === 0 ? (
                    ...
                    filteredTokens.map((opt, idx) => (
                      ...
                      >
                        ...
                        {JSON.stringify(selectedToken) === JSON.stringify(opt) && (
            ...
            <div className="p-5 flex-1 overflow-y-auto">
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                  ...
                  <select
                    className="w-full bg-muted border border-border rounded-xl py-2.5 pl-9 pr-9 font-bold text-foreground text-xs appearance-none focus:outline-none focus:border-primary/50"
                    value={withdrawToken}
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  placeholder={`Enter ${withdrawToken} address`}
                  className="w-full bg-muted border border-border rounded-xl px-3 py-2.5 text-xs text-foreground focus:outline-none focus:border-primary/50"
                />
              </div>
                ...
                return (
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      placeholder="0.00"
                      className="w-full bg-muted border border-border rounded-xl px-3 py-2.5 font-mono text-base text-foreground focus:outline-none focus:border-primary/50 mb-2"
                    />
              ...
              <div className="mb-5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-muted border border-border rounded-xl pl-9 pr-3 py-2.5 text-xs text-foreground focus:outline-none focus:border-primary/50"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5 relative">
                <div>
                  ...
                  <div className="bg-muted border border-border rounded-lg pl-3 py-2 text-xs font-bold text-foreground">
                    {transferFrom === "Spot" ? "Spot Wallet" : "Futures Wallet"}
                  </div>
                </div>
                <div className="flex justify-center -my-1.5 relative z-10">
                  <button
                    onClick={() => setTransferFrom(transferFrom === "Spot" ? "Futures" : "Spot")}
                    className="bg-muted border border-border rounded-full p-1.5 shadow-sm hover:bg-accent text-muted-foreground"
                  >
                    <ArrowDown size={14} />
                  </button>
                </div>
                <div>
                  ...
                  <div className="bg-muted border border-border rounded-lg pl-3 py-2 text-xs font-bold text-foreground">
                    {transferFrom === "Spot" ? "Futures Wallet" : "Spot Wallet"}
                  </div>
                </div>
              </div>
