...
const Spot = () => {
  ...
  // Save orders helper
  const saveSpotOrders = useCallback(
    (newOrders: SpotOrder[]) => {
        ...
        Object.entries(liveUpdates).forEach(([pair, data]) => {
          ...
          if (!next[s] || next[s].lastPrice !== data.price) {
            next[s] = {
              ...
              priceChangePercent:
                data.change24h !== undefined ? data.change24h : next[s]?.priceChangePercent || 0,
            };
            changed = true;
          }
        });
    ...
    for (let i = 0; i < updatedOrders.length; i++) {
      ...
      if (order.side === "BUY" && livePrice <= order.price) {
        ...
        if (existingAssetIndex >= 0) {
          ...
          nextAssets = userAssets.map((a, idx) =>
            idx === existingAssetIndex ? { ...a, amount: newAmt } : a,
          );
        ...
        toast.success(
          `🎉 Limit Buy Order Filled: Bought ${order.amount.toFixed(4)} ${order.symbol} @ $${livePrice.toFixed(2)}`,
        );
      }
      ...
      if (order.side === "SELL" && livePrice >= order.price) {
        ...
        toast.success(
          `🎉 Limit Sell Order Filled: Sold ${order.amount.toFixed(4)} ${order.symbol} for $${order.total.toFixed(2)} USDT`,
        );
      }
    }
    ...
    if (side === "BUY") {
      ...
      if (type === "MARKET") {
        ...
        if (existingAssetIndex >= 0) {
          ...
          updatedAssets = userAssets.map((a, idx) =>
            idx === existingAssetIndex ? { ...a, amount: newAmt } : a,
          );
          ...
          await supabase
            .from("user_assets")
            .insert({ user_id: user.id, symbol: baseSymbol, amount: Number(amount.toFixed(8)) });
        }
      ...
      if (newBaseAmt < 0.000001) {
        ...
        updatedAssets = userAssets.map((a) =>
          a.id === existingAsset.id ? { ...a, amount: newBaseAmt } : a,
        );
        await supabase
          .from("user_assets")
          .update({ amount: newBaseAmt })
  ...
  const handleCancelOrder = async (orderId: string) => {
    ...
    const updatedOrders = spotOrders.map((o) =>
  ...
  const handleConvertSubmit = async () => {
    ...
    if (convertFrom === "USDT") {
      ...
      const { data: dbAsset } = await supabase
        ...
        .select("*")
        .eq("user_id", user.id)
        .eq("symbol", convertFrom)
        .maybeSingle();
      if (!dbAsset || amount > Number(dbAsset.amount || 0)) {
      ...
      if (dbAsset) {
        ...
        if (dustValueInUSDT < 0.1 || newAmt.toFixed(4) === "0.0000") {
          ...
          await supabase
            ...
            .eq("symbol", convertFrom);
          setUserAssets((prev) =>
            prev.map((a) => (a.symbol === convertFrom ? { ...a, amount: newAmt } : a)),
          );
        }
      }
    }
    ...
    if (convertTo === "USDT") {
      ...
      if (dbAsset) {
        ...
        await supabase
          ...
          .eq("symbol", convertTo);
        setUserAssets((prev) =>
          prev.map((a) => (a.symbol === convertTo ? { ...a, amount: newAmt } : a)),
        );
      } else {
        await supabase
          .from("user_assets")
          .insert({ user_id: user.id, symbol: convertTo, amount: toAmount });
      }
    }
  ...
  const activeTicker = tickers[selectedPair] || {
    lastPrice: 0,
  ...
  const previewFromPrice =
    convertFrom === "USDT" ? 1 : marketService.getCurrentPrice(convertFrom + "USDT");
  const previewToPrice =
    convertTo === "USDT" ? 1 : marketService.getCurrentPrice(convertTo + "USDT");
  const previewToAmount =
    (parseFloat(convertAmount || "0") * previewFromPrice) / (previewToPrice || 1);
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:flex items-center gap-4 text-xs pl-2 border-l border-border/80">
            <div>
              ...
              >
                $
                {activeTicker.lastPrice.toLocaleString(undefined, {
            ...
            <div>
              ...
              <div
                className={`font-mono font-bold ${activeTicker.priceChangePercent >= 0 ? "text-success" : "text-danger"}`}
              >
          ...
          <div className="bg-card w-full max-w-sm rounded-[24px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-scale-in border border-border">
            ...
            <div className="p-5 flex-1 overflow-y-auto space-y-3">
                ...
                <div className="flex items-center gap-2">
                  <div className="relative shrink-0 flex items-center gap-1 bg-card px-2 py-1 rounded-lg border border-border">
                    ...
                    <select
                      ...
                      className="bg-transparent font-bold text-xs outline-none w-16 appearance-none z-10 relative pr-4 text-foreground"
                    >
                      {ALL_ASSETS.filter((a) => a !== convertTo).map((asset) => (
                  ...
                  <input
                    ...
                    className="flex-1 min-w-0 bg-transparent text-right font-mono text-base font-bold outline-none text-foreground"
                    placeholder="0.00"
                  />
                </div>
                <div className="flex justify-end items-center gap-2 mt-1.5">
                  <span className="text-[11px] text-muted-foreground font-bold uppercase truncate">
                    ...
                    {convertFrom === "USDT"
                      ...
                      : (userAssets.find((a) => a.symbol === convertFrom)?.amount || 0).toFixed(4)}
                  </span>
                  <button
                    onClick={() => {
                      const max =
              ...
              <div className="flex justify-center -my-1.5 relative z-10">
                <button
                  onClick={handleConvertSwap}
                  className="bg-card border border-border p-1.5 rounded-full shadow-sm hover:bg-muted transition-all text-primary"
                >
                  <ArrowDown size={14} />
                </button>
              </div>
                ...
                <div className="flex items-center gap-2">
                  <div className="relative shrink-0 flex items-center gap-1 bg-card px-2 py-1 rounded-lg border border-border">
                    ...
                    <select
                      ...
                      className="bg-transparent font-bold text-xs outline-none w-16 appearance-none z-10 relative pr-4 text-foreground"
                    >
                      {ALL_ASSETS.filter((a) => a !== convertFrom).map((asset) => (
                        <option key={asset} value={asset}>
                          {asset}
                        </option>
                      ))}
                  ...
                  <input
                    disabled
                    value={previewToAmount > 0 ? previewToAmount.toFixed(6) : ""}
                    className="flex-1 min-w-0 bg-transparent text-right font-mono text-base font-bold outline-none text-muted-foreground"
                    placeholder="0.00"
                    title="Resulting amount"
                  />
                </div>
              </div>
              ...
              <div className="bg-muted p-3 rounded-xl border border-border text-[11px]">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-foreground">
                    1 {convertFrom} ≈ {(previewFromPrice / (previewToPrice || 1)).toFixed(6)}{" "}
                    {convertTo}
                  </span>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-3 rounded-xl shadow-sm mt-2 disabled:opacity-50 transition-all text-sm"
              >
                Confirm Conversion
              </button>
            </div>
          ...
          <div className="bg-card w-full max-w-sm rounded-[24px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-scale-in border border-border">
            ...
            <div className="p-5 flex-1 overflow-y-auto">
              <div className="space-y-1.5 relative">
                <div>
                  ...
                  <div className="bg-muted border border-border rounded-lg pl-3 py-2 text-xs font-bold text-foreground">
                    {transferFromSpot ? "Spot Wallet" : "Futures Wallet"}
                  </div>
                </div>
                <div className="flex justify-center -my-1.5 relative z-10">
                  <button
                    onClick={() => setTransferFromSpot(!transferFromSpot)}
                    className="bg-card border border-border rounded-full p-1.5 shadow-sm hover:bg-muted text-muted-foreground"
                  >
                    <ArrowDown size={14} />
                  </button>
                </div>
              ...
              <button
                ...
                className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-3.5 rounded-xl shadow-sm transition-all disabled:opacity-50 text-sm"
              >
