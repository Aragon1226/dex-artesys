...
const LEVERAGE_CONFIG: Record<number, { duration: number; profit: number }> = {
  25: { duration: 120, profit: 0.175 },
  50: { duration: 240, profit: 0.35 },
  75: { duration: 450, profit: 0.7 },
  100: { duration: 720, profit: 0.99 },
};
...
const Futures = () => {
  ...
  const activeTicker = tickers[selectedPair.symbol] || {
    lastPrice: marketService.getCurrentPrice(selectedPair.symbol),
    priceChangePercent: 0,
  };
  const currentPrice =
    activeTicker.lastPrice || marketService.getCurrentPrice(selectedPair.symbol) || 73500;
    ...
    const interval = setInterval(async () => {
      ...
      const updated = await Promise.all(
        currentPositions.map(async (pos) => {
          ...
          if (elapsed >= pos.duration_seconds) {
            ...
            const profitAmt = isWin
              ? pos.margin * (pos.expected_profit_percentage || 0)
              : -pos.margin;
            try {
              ...
              await supabase
                .from("positions")
                .update({ status: "CLOSED", pnl: profitAmt })
                .eq("id", pos.id);
              ...
              await supabase
                .from("profiles")
                .update({ futures_balance: (prof?.futures_balance || 0) + pos.margin + profitAmt })
                .eq("id", user.id);
            }
      ...
      if (hasChanges) {
        ...
        setTimeout(async () => {
          ...
          if (data) {
            updateProfileLocally(data as UserProfile);
  ...
  const handleOpenTrade = async (side: "LONG" | "SHORT") => {
    ...
    if (margin > (profile.futures_balance || 0)) {
    ...
    const newPosition: FuturePosition = {
      ...
      expected_profit_percentage: config.profit,
      status: "OPEN",
      pnl: 0,
    ...
    try {
      ...
      await supabase.from("positions").insert({
        ...
        duration_seconds: newPosition.duration_seconds,
  ...
  const handleTransfer = async () => {
    ...
    if (transferFromSpot && amount > (profile.balance || 0)) {
      toast.error("Insufficient Spot Balance");
      return;
    }
    if (!transferFromSpot && amount > (profile.futures_balance || 0)) {
      toast.error("Insufficient Futures Balance");
      return;
    }
    setIsTransferring(true);
    const newSpot = transferFromSpot
      ? (profile.balance || 0) - amount
      : (profile.balance || 0) + amount;
    const newFutures = transferFromSpot
      ? (profile.futures_balance || 0) + amount
      : (profile.futures_balance || 0) - amount;
    try {
      await supabase
        .from("profiles")
        .update({ balance: newSpot, futures_balance: newFutures })
        .eq("id", user.id);
    } catch (e) {
  ...
  const calculateProgress = (pos: FuturePosition) =>
    Math.min(((currentTime - pos.start_time) / 1000 / pos.duration_seconds) * 100, 100);
  const formatTimeRemaining = (pos: FuturePosition) =>
    `${Math.max(0, Math.floor(pos.duration_seconds - (currentTime - pos.start_time) / 1000))}s`;
          ...
          <div className="hidden lg:block bg-card rounded-xl p-4 shadow-sm border border-border">
            <div className="flex items-center gap-4 mb-3 border-b border-border">
              <button
                ...
                className={`text-[11px] font-bold flex items-center gap-1.5 pb-2 border-b-2 transition-all ${positionTab === "active" ? "text-foreground border-primary" : "text-muted-foreground border-transparent"}`}
              >
                <Activity size={14} /> Active
              </button>
              <button
                onClick={() => setPositionTab("history")}
                className={`text-[11px] font-bold flex items-center gap-1.5 pb-2 border-b-2 transition-all ${positionTab === "history" ? "text-foreground border-primary" : "text-muted-foreground border-transparent"}`}
              >
              ...
              positionsLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {activePositions.map((pos) => {
                    ...
                    const basePnlAmount = displayWin
                      ? pos.margin * (pos.expected_profit_percentage || 0)
                      : -pos.margin;
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-4">
                          <div
                            className={`w-5 h-5 flex items-center justify-center rounded text-[11px] font-bold text-white ${pos.type === "LONG" ? "bg-success" : "bg-danger"}`}
                          >
                            {pos.type === "LONG" ? "L" : "S"}
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4">
                          <div>
                            ...
                            <div
                              className={`text-base font-black ${pnlColor} transition-colors duration-300`}
                            >
                              {currentPnlAmount >= 0 ? "+" : ""}
                              {currentPnlAmount.toFixed(2)}
                            </div>
                          </div>
                          ...
                          <div className="text-right">
                            ...
                            <div className="text-sm font-bold text-foreground transition-all duration-300">
                              {currentSimulatedPrice.toLocaleString(undefined, {
              ...
              <div className="space-y-1.5">
                {closedPositions.map((pos) => {
                  ...
                  return (
                    <div
                      key={pos.id}
                      className="bg-muted/30 border border-border rounded-lg p-2.5 flex justify-between items-center hover:bg-muted transition-colors"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-1.5 mb-0.5">
                            ...
                            <span
                              className={`text-[8px] font-black px-1 py-0.5 rounded ${pos.type === "LONG" ? "bg-success/10 text-success" : "bg-danger/10 text-danger"}`}
                            >
                      ...
                      <div className="text-right">
                        <div
                          className={`font-black text-xs ${pnl >= 0 ? "text-success" : "text-danger"}`}
                        >
                          {pnl >= 0 ? "+" : ""}
                          {pnl.toFixed(2)} USDT
                        </div>
          ...
          <div className="bg-card rounded-xl p-3 shadow-sm border border-border sticky top-3">
            ...
            <div className="bg-muted rounded-lg p-2.5 border border-border mb-3">
              <div className="flex justify-between items-center mb-0.5">
                <span className="text-[8px] font-bold uppercase tracking-tight text-muted-foreground font-mono">
                  Wallet
                </span>
                <button
                ...
                >
                  {balanceHidden ? <EyeOff size={10} /> : <Eye size={10} />}
                </button>
              </div>
              <div className="text-base font-black text-foreground font-mono">
                {balanceHidden
                  ? "••••••"
                  : (profile.futures_balance || 0).toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}{" "}
            ...
            <div className="mb-4">
              <div className="flex justify-between items-center mb-1">
                <label className="text-[8px] font-bold uppercase tracking-tight text-muted-foreground">
                  Amount
                </label>
              ...
              <div className="grid grid-cols-3 gap-1 mt-1.5">
                {[0.25, 0.5, 1].map((pct) => (
                  <button
                    key={pct}
                    onClick={() =>
                      setMarginInput(((profile.futures_balance || 0) * pct).toFixed(2))
                    }
                    className="py-1 rounded text-[8px] font-bold bg-card text-muted-foreground hover:bg-muted border border-border transition-all uppercase"
                  >
                    {pct * 100}%
                  </button>
                ))}
                  ...
                  <div key={pos.id} className="bg-muted border border-border rounded-lg p-3">
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div
                            className={`text-[8px] font-bold inline-block px-1 rounded ${pos.type === "LONG" ? "bg-success/10 text-success" : "bg-danger/10 text-danger"}`}
                          >
                            {pos.type} {pos.leverage}x
                          </div>
                        </div>
                      ...
                      <div className="text-right">
                        <div
                          className={`text-[11px] font-bold ${pnl >= 0 ? "text-success" : "text-danger"}`}
                        >
                          {pnl >= 0 ? "+" : ""}
                          {pnl.toFixed(2)} USDT
                        </div>
                    ...
                    <div className="flex flex-col gap-1 mt-2 border-t border-border/50 pt-2 text-[10px] text-muted-foreground">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>
                          Close:{" "}
                          {closePrice.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="bg-card w-full max-w-md rounded-[32px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-scale-in border border-border">
            ...
            <div className="p-6 flex-1 overflow-y-auto">
              <div className="space-y-2 relative">
                <div>
                  <label className="text-sm font-bold text-foreground mb-2 block">From</label>
                  <div className="bg-muted border border-border rounded-xl pl-4 py-3.5 font-bold text-foreground">
                    {transferFromSpot ? "Spot Account" : "Futures Account"}
                  </div>
                </div>
                <div className="flex justify-center -my-1 relative z-10">
                  <button
                    onClick={() => setTransferFromSpot(!transferFromSpot)}
                    className="bg-card border border-border rounded-full p-2 shadow-sm hover:bg-muted text-muted-foreground"
                  >
                    <ArrowDown size={18} />
                  </button>
                </div>
                <div>
                  <label className="text-sm font-bold text-foreground mb-2 block">To</label>
                  <div className="bg-muted border border-border rounded-xl pl-4 py-3.5 font-bold text-foreground">
                    {!transferFromSpot ? "Spot Account" : "Futures Account"}
                  </div>
                </div>
              ...
              <div className="mt-6 mb-8">
                <div className="flex justify-between mb-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    Available:{" "}
                    {(transferFromSpot ? profile.balance : profile.futures_balance || 0).toFixed(2)}{" "}
                    USDT
                  </span>
                </div>
                <input
                  ...
                  placeholder="0.00"
                  className="w-full bg-muted border border-border rounded-xl pl-4 pr-16 py-3.5 font-mono text-lg text-foreground focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none"
                />
              </div>
              <button
                onClick={handleTransfer}
                disabled={!transferAmount || parseFloat(transferAmount) <= 0 || isTransferring}
                className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-4 rounded-xl shadow-brand transition-all disabled:opacity-50"
              >
              ...
              {PAIRS.map((pair) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-2.5 rounded-xl transition-colors hover:bg-muted ${selectedPair.symbol === pair.symbol ? "bg-primary/10 border border-primary/30" : "border border-transparent"}`}
                  >
                    <div className="flex items-center gap-3">
                    ...
                    <div className="text-right">
                      <div className="font-bold text-foreground font-mono text-xs">
                        $
                        {pt.lastPrice.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </div>
          ...
          <div className="bg-card w-full max-w-sm rounded-3xl p-6 shadow-2xl text-center animate-scale-in">
            ...
            <div className="bg-muted rounded-xl p-4 mb-6 text-left border border-border">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="font-bold text-foreground">{newPositionModal.pair}</span>
              </div>
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm text-muted-foreground">Margin</span>
                <span className="font-bold text-foreground">
                  ${newPositionModal.margin.toFixed(2)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-success">
                  +$
                  {(
                    newPositionModal.margin *
                    (1 + (newPositionModal.expected_profit_percentage || 0))
                  ).toFixed(2)}
                </span>
              </div>
            </div>
            <button
              onClick={() => setNewPositionModal(null)}
              className="w-full bg-foreground text-background font-bold py-3.5 rounded-xl transition-colors hover:bg-foreground/90"
            >
              Close & Monitor
            </button>
          </div>
