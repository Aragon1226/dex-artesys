...
const Earn = () => {
  ...
  const blendedApr = stakeType === 'fixed' ? (FIXED_DURATIONS.find(d => d.days === duration)?.apr ?? 5) : 0.25;
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="rounded-2xl p-5 border border-primary/20 bg-gradient-to-br from-primary/10 via-card to-card shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-muted-foreground text-[11px] font-bold uppercase tracking-widest mb-1">Total Earn Balance</p>
              <h2 className="text-3xl font-bold text-foreground font-mono tracking-tight">${staked.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h2>
              <p className="text-[10px] font-bold text-success mt-1">+${dailyYield.toFixed(2)} / day est.</p>
            </div>
          ...
          <div className="grid grid-cols-3 gap-2 mt-5 pt-4 border-t border-border/60">
            {[
              ...
              <div key={s.label}>
                <p className="text-[9px] uppercase tracking-wider text-muted-foreground font-bold mb-1">{s.label}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div onClick={() => { setStakeType('fixed'); setAmount(''); setShowStake(true); }} className="rounded-2xl p-4 border border-warning/25 bg-warning/5 cursor-pointer active:scale-[0.98] transition-transform hover:border-warning/50">
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-9 h-9 rounded-xl bg-warning/15 text-warning flex items-center justify-center"><Lock size={15} /></div>
              <div><h3 className="text-sm font-bold text-foreground">Fixed Term</h3><p className="text-[10px] text-muted-foreground">Locked period, higher yield</p></div>
            </div>
            <div className="flex justify-between items-end">
              <div><p className="text-[10px] text-muted-foreground">APR up to</p><p className="text-base font-bold text-success font-mono">9.80%</p></div>
              <div className="text-right"><p className="text-[10px] text-muted-foreground">Terms</p><p className="text-[11px] font-bold text-foreground">10d – 180d</p></div>
            </div>
          </div>

          <div onClick={() => { setStakeType('flexible'); setAmount(''); setShowStake(true); }} className="rounded-2xl p-4 border border-primary/20 bg-primary/5 cursor-pointer active:scale-[0.98] transition-transform hover:border-primary/50">
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-9 h-9 rounded-xl bg-primary/15 text-primary flex items-center justify-center"><Clock size={15} /></div>
              <div><h3 className="text-sm font-bold text-foreground">Flexible</h3><p className="text-[10px] text-muted-foreground">Redeem at any time</p></div>
            </div>
            <div className="flex justify-between items-end">
              <div><p className="text-[10px] text-muted-foreground">APR</p><p className="text-base font-bold text-success font-mono">0.25%</p></div>
              <div className="text-right"><p className="text-[10px] text-muted-foreground">Lock Period</p><p className="text-[11px] font-bold text-foreground">None</p></div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card overflow-hidden">
          <div className="px-4 py-3 border-b border-border flex items-center justify-between">
            <h3 className="text-[11px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-1.5"><Coins size={13} className="text-primary" /> Earn Products</h3>
            <span className="text-[9px] font-bold text-muted-foreground font-mono">Updated hourly</span>
          </div>
          <div className="divide-y divide-border">
            {EARN_PRODUCTS.map((prod) => (
              <button
                key={prod.symbol}
                onClick={() => { setStakeType('fixed'); setAmount(''); setShowStake(true); }}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs font-bold text-foreground font-mono">{prod.symbol}</span>
                    <span className="text-[8px] font-extrabold uppercase px-1.5 py-0.5 rounded bg-muted text-muted-foreground">{prod.tag}</span>
                  </div>
                  <p className="text-[10px] text-muted-foreground truncate">{prod.name} · Min {prod.min}</p>
                  <div className="h-1 mt-1.5 w-24 rounded-full bg-muted overflow-hidden">
                    <div className="h-full rounded-full bg-primary/70" style={{ width: `${prod.capacity}%` }} />
                  </div>
                </div>
        ...
        {staked > 0 ? (
          <div className="rounded-2xl border border-border bg-card p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-[11px] font-black uppercase tracking-widest text-muted-foreground">Active Position</h3>
              <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-success/10 text-success uppercase">Accruing</span>
            </div>
            <div className="flex items-center gap-3">
              <CryptoIcon symbol="USDT" size={36} />
              <div className="flex-1">
                <p className="text-sm font-bold text-foreground font-mono">${staked.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                <p className="text-[10px] text-muted-foreground">Flexible savings · settles daily at 00:00 UTC</p>
              </div>
              <button onClick={() => { setTransferFromSpot(false); setAmount(''); setShowTransfer(true); }} className="px-3 py-1.5 rounded-lg border border-border text-[10px] font-bold text-foreground hover:bg-muted">Redeem</button>
            </div>
          </div>
        ) : (
          <div className="rounded-2xl border border-dashed border-border bg-card/60 p-8 text-center flex flex-col items-center">
            <div className="w-12 h-12 rounded-2xl bg-primary/10 text-primary flex items-center justify-center mb-3"><Wallet size={20} /></div>
            <h3 className="text-sm font-bold text-foreground mb-1">No Active Positions</h3>
            <p className="text-[11px] text-muted-foreground max-w-[220px] mb-4">Move idle balance into Earn and start compounding rewards daily.</p>
            <button onClick={() => { setStakeType('fixed'); setAmount(''); setShowStake(true); }} className="px-4 py-2 rounded-xl bg-primary text-primary-foreground text-[11px] font-bold">Start Earning</button>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div key={item.label} className="rounded-xl border border-border bg-card px-2 py-3 text-center">
              <item.icon size={14} className="text-primary mx-auto mb-1.5" />
              <p className="text-[9px] font-bold text-muted-foreground leading-tight">{item.label}</p>
            </div>
          ))}
        </div>
        ...
        <p className="flex items-start gap-1.5 text-[9px] text-muted-foreground leading-relaxed px-1">
          ...
          Displayed APRs are estimates based on current network conditions and may change. Rewards are simulated for demonstration purposes.
        </p>
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center backdrop-blur-md p-4">
          <div className="bg-card w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-scale-in border border-border">
            ...
            <div className="p-5 flex-1 overflow-y-auto">
              <div className={`rounded-lg p-3 mb-4 flex items-center justify-between ${stakeType === 'fixed' ? 'bg-warning/5 border border-warning/25' : 'bg-primary/5 border border-primary/20'}`}>
                <div><span className="text-[10px] font-bold text-muted-foreground uppercase block mb-0.5">APR Rate</span><span className="text-lg font-bold text-success font-mono">{blendedApr.toFixed(2)}%</span></div>
              ...
              {stakeType === 'fixed' && (
                <div className="mb-4"><label className="text-[11px] font-bold text-foreground mb-1.5 block">Duration</label><div className="grid grid-cols-4 gap-1.5">{FIXED_DURATIONS.map(d => (
                  <button key={d.days} type="button" onClick={() => setDuration(d.days)} className={`py-1.5 rounded-lg text-xs font-bold border transition-all ${duration === d.days ? 'bg-primary text-primary-foreground border-primary' : 'bg-card text-muted-foreground border-border hover:border-muted-foreground/30'}`}>{d.label}</button>
                ))}</div></div>
              )}
              <div className="mb-6">
