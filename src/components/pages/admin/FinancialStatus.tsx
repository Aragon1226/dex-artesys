...
import { Search, RefreshCw, Coins, ArrowUp, ArrowDown, X, Wallet } from "lucide-react";
...
const FinancialStatus = () => {
  ...
  const handleAdjustBalance = async (e: React.FormEvent) => {
    ...
    try {
        ...
        prev.map((u) => {
          if (u.id === selectedUserForBalance.id) {
            return {
              ...u,
              [fieldKey]: newVal,
              total_value: (u.total_value || 0) - currentVal + newVal,
            };
          }
          return u;
        }),
      ...
      try {
        const walletLabel =
          balanceWalletType === "spot"
            ? "Spot Wallet"
            : balanceWalletType === "futures"
              ? "Futures Wallet"
              : "Staking/Earn Wallet";
        const actionLabel =
      ...
      toast.success(
        `Successfully adjusted balance for ${selectedUserForBalance.username || selectedUserForBalance.email}`,
      );
    ...
    .sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "balance":
          ...
          break;
        case "futures":
          res = (a.futures_balance ?? 0) - (b.futures_balance ?? 0);
          break;
        case "earn":
          res = (a.staked_balance ?? 0) - (b.staked_balance ?? 0);
          break;
        case "total":
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage user balances and view withdrawal addresses.
          </p>
        </div>
        <button
          onClick={() => loadData()}
                ...
                <thead className="bg-muted/50 border-b border-border">
                  <tr>
                    <th
                      ...
                      className="px-6 py-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider cursor-pointer hover:bg-muted select-none"
                    >
                    ...
                    >
                      <div className="flex items-center gap-1">
                        Spot Balance <SortIcon field="balance" />
                      </div>
                    </th>
                    <th
                    ...
                    >
                      <div className="flex items-center gap-1">
                        Futures <SortIcon field="futures" />
                      </div>
                    </th>
                    <th
                      onClick={() => handleSort("earn")}
                      className="px-6 py-4 text-xs font-semibold text-muted-foreground uppercase tracking-wider cursor-pointer hover:bg-muted select-none"
                    >
                      <div className="flex items-center gap-1">
                        Earn <SortIcon field="earn" />
                      </div>
                    </th>
                    <th
                    ...
                    >
                      <div className="flex items-center gap-1">
                        Total Value <SortIcon field="total" />
                      </div>
                    </th>
                    ...
                    <tr key={user.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm font-bold text-indigo-500 font-mono">
                          $
                          {(user.futures_balance ?? 0).toLocaleString("en-US", {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                          })}
                        </div>
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm font-bold text-warning font-mono">
                          $
                          {(user.staked_balance ?? 0).toLocaleString("en-US", {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                          })}
                        </div>
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm font-bold text-foreground font-mono">
                          $
                          {(user.total_value ?? 0).toLocaleString("en-US", {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                          })}
                        </div>
                      ...
                      <td className="px-6 py-4 text-sm font-mono text-muted-foreground break-all max-w-[200px]">
                        {user.withdrawal_address ? (
                          <div className="bg-muted p-2 rounded-lg text-xs break-all">
                            {user.withdrawal_address}
                          </div>
                        ) : (
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  <span className="font-semibold text-foreground">
                    {selectedUserForBalance.username || selectedUserForBalance.email}
                  </span>
                </p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  { id: "spot" as const, label: "Spot", val: selectedUserForBalance.balance },
                  {
                    id: "futures" as const,
                    label: "Futures",
                    val: selectedUserForBalance.futures_balance,
                  },
                  {
                    id: "staked" as const,
                    label: "Earn",
                    val: selectedUserForBalance.staked_balance,
                  },
                ].map((w) => (
                  <button
                    key={w.id}
                ...
                {[
                  {
                    id: "add" as const,
                    label: "Add (Credit)",
                    style: "hover:border-success/50 hover:text-success active:bg-success/10",
                  },
                  {
                    id: "deduct" as const,
                    label: "Deduct (Debit)",
                    style: "hover:border-danger/50 hover:text-danger active:bg-danger/10",
                  },
                  {
                    id: "set" as const,
                    label: "Set (Override)",
                    style: "hover:border-primary/50 hover:text-primary active:bg-primary/10",
                  },
                ].map((op) => (
                  <button
                    key={op.id}
            ...
            <div className="mb-6">
              ...
              <div className="relative">
                <input
                  ...
                  required
                />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 font-bold text-xs uppercase tracking-wider text-muted-foreground">
                  USDT
                </span>
              </div>
            </div>
