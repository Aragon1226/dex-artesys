        ...
        if (deposit.asset === "USDT") {
          ...
          if (assetRow) {
            ...
            const { error: uErr } = await supabase
              .from("user_assets")
              .update({ amount: currentAmount + depositAmt })
              .eq("id", assetRow.id);
            if (uErr) throw uErr;
          } else {
            const { error: iErr } = await supabase
              .from("user_assets")
              .insert({ user_id: deposit.user_id, symbol: deposit.asset, amount: depositAmt });
            if (iErr) throw iErr;
          }
        }
      }
  ...
  if (error) {
    return (
      <div className="h-[80vh] flex flex-col items-center justify-center p-6 text-center">
        ...
        <button
          onClick={() => loadData()}
          className="px-6 py-3 bg-primary text-primary-foreground font-bold rounded-2xl"
        >
          Try Again
        </button>
      </div>
    ...
    <div className="p-6 lg:p-10 space-y-8 max-w-[1600px] mx-auto">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => loadData()}
            className="group flex items-center gap-2 px-4 py-2 bg-card border border-border rounded-xl text-sm font-bold text-muted-foreground hover:text-primary transition-all shadow-sm"
          >
            <RefreshCw
              size={16}
        ...
        {deposits.length === 0 ? (
          ...
          <div className="divide-y divide-border">
            {deposits.map((dep) => (
              <div
                key={dep.id}
                className="p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 hover:bg-muted/10 transition-colors"
              >
      ...
      {confirmingAction && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-3xl p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200">
            <div
              className={`w-16 h-16 rounded-full mx-auto mb-4 flex items-center justify-center ${confirmingAction.action === "APPROVED" ? "bg-success/10 text-success" : "bg-danger/10 text-danger"}`}
            >
