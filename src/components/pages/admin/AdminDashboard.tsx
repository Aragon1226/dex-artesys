...
const AdminDashboard = () => {
  ...
  const handleDepositAction = async () => {
    ...
    console.log(
      `[AdminDashboard] Starting ${action} for deposit ${id} (User: ${userId}, Amount: ${amount} ${asset})`,
    );
    ...
    try {
      ...
      if (action === "APPROVED") {
        ...
        if (asset === "USDT") {
          ...
          if (balanceError) {
            console.error(
              "[AdminDashboard] Supabase balanceError (profiles update):",
              balanceError,
            );
            throw balanceError;
          }
          ...
          setProfiles((prev) =>
            prev.map((p) => (p.id === userId ? { ...p, balance: newBalance } : p)),
          );
        } else {
  ...
  const handleSetOutcome = async (
    ...
    if (
      profile.force_win === (updates as any).force_win &&
      profile.force_loss === (updates as any).force_loss
    )
      return;
    ...
    <div className="p-6 lg:p-10 space-y-8 max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Link
          ...
          className="block transform transition-transform hover:scale-[1.02] active:scale-98"
        >
          <StatCard
        ...
        <div className="space-y-8">
          ...
          <div className="bg-card rounded-3xl border border-border overflow-hidden shadow-sm">
            ...
            <div className="divide-y divide-border">
              {forcedOutcomeUsers.length === 0 ? (
                <div className="p-12 text-center text-muted-foreground text-sm opacity-50 italic">
                  No manual trade overrides active.
                </div>
              ) : (
                forcedOutcomeUsers.slice(0, 5).map((u) => (
                  <div
                    key={u.id}
                    className="px-8 py-4 flex items-center justify-between hover:bg-muted/10 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs ${u.force_win ? "bg-success/10 text-success" : u.force_loss ? "bg-danger/10 text-danger" : "bg-primary/10 text-primary"}`}
                      >
                    ...
                    >
                      <span
                        className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${u.force_win ? "translate-x-5" : "translate-x-1"}`}
                      />
                    </button>
                  </div>
                ))
          ...
          <div className="bg-card rounded-3xl border border-border overflow-hidden shadow-sm">
            <div className="px-8 py-5 border-b border-border">
              <h3 className="font-black text-foreground uppercase tracking-widest text-xs opacity-70">
            ...
            <div className="divide-y divide-border">
              {newestUsers.map((user) => (
                <div
                  key={user.id}
                  className="px-8 py-4 flex justify-between items-center hover:bg-muted/10 transition-colors"
                >
                  <div>
      ...
      {confirmDeposit && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-3xl p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200">
            <div
              className={`w-16 h-16 rounded-full mx-auto mb-4 flex items-center justify-center ${confirmDeposit.action === "APPROVED" ? "bg-success/10 text-success" : "bg-danger/10 text-danger"}`}
            >
              {confirmDeposit.action === "APPROVED" ? <Check size={32} /> : <X size={32} />}
            </div>
            <h3 className="text-xl font-bold text-foreground text-center mb-2">
              Confirm {confirmDeposit.action}
            </h3>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className={`flex-1 py-3 rounded-2xl font-bold text-white shadow-xl transition-all ${
                  confirmDeposit.action === "APPROVED"
                    ? "bg-success shadow-success/20 hover:bg-success"
                    : "bg-danger shadow-danger/20 hover:bg-danger"
                }`}
              >
