...
const AdminUsers = () => {
  ...
  const [deleteReason, setDeleteReason] = useState<string>(
    "Client requested permanent account closure",
  );
  ...
  const [selectedUserForAdminAssign, setSelectedUserForAdminAssign] = useState<ProfileRow | null>(
    null,
  );
  ...
  const handleAssignUserAdmin = async (e: React.FormEvent) => {
    ...
    try {
      setReferrerForUser(
        selectedUserForAdminAssign.email,
        selectedUserForAdminAssign.id,
        targetAdminId,
      );
  ...
  const handleBanUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        `Account ${selectedUserForBan.email || selectedUserForBan.username} suspended (${banType === "client_request" ? "Client Request" : "Forced"}).`,
      );
  ...
  const handleUnbanUser = async () => {
    ...
    try {
      ...
      toast.success(
        `Account ${selectedUserForUnban.email || selectedUserForUnban.username} restored and unbanned successfully.`,
      );
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      const res = await deleteUserAccountComplete(
      ...
      toast.success(
        `Account ${selectedUserForDelete.email || selectedUserForDelete.username} permanently deleted (${deleteMode === "client_request" ? "Client Request" : "Administrative Force"}).`,
      );
  ...
  const loadUsers = useCallback(async (silent = false) => {
    ...
    setError(null);
    try {
      const { data: profs, error: fetchError } = await supabase
        .from("profiles")
        .select("*")
        .order("created_at", { ascending: false });
      ...
      if (filteredProfs) {
        const enriched = filteredProfs.map((p) => {
          ...
          const cryptoVal = userAssets.reduce((sum, a) => {
            ...
            const price =
              allPrices[a.symbol] || marketService.getCurrentPrice(a.symbol + "USDT") || 0;
            return sum + a.amount * price;
          }, 0);
          return {
            ...p,
            total_value:
              (p.balance || 0) + (p.futures_balance || 0) + (p.staked_balance || 0) + cryptoVal,
          };
        });
  ...
  const isE2EUser = (u: ProfileRow) => {
    ...
    return (
      un.startsWith("e2e") || em.startsWith("e2e") || em.includes("e2e_") || em.includes("e2e-")
    );
  };
  ...
  const handlePurgeAllE2EAccounts = async () => {
    ...
    try {
      ...
      toast.success(
        `Successfully purged ${successCount} E2E test account(s) from database & auth.`,
      );
      setShowE2EPurgeConfirm(false);
      await loadUsers(true);
    } catch (err: any) {
  ...
  const processed = users
    ...
    .sort((a, b) => {
      ...
      switch (sortField) {
        case "balance":
          res = (a.balance ?? 0) - (b.balance ?? 0);
          break;
        case "total_value":
          res = (a.total_value ?? 0) - (b.total_value ?? 0);
          break;
        case "joined":
        ...
        case "kyc":
          res =
            (kycOrder[a.kyc_status || "UNVERIFIED"] || 0) -
            (kycOrder[b.kyc_status || "UNVERIFIED"] || 0);
          break;
        case "name":
    ...
    <div className="p-6 lg:p-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-success">
                Security Audit Portal (Developer Mode Active)
              </h4>
              <p className="text-xs text-muted-foreground">
                Monitoring active sessions, connection protocols, and user access IPs for all
                registered users.
              </p>
            </div>
          </div>
          ...
          <>
                  ...
                  <tbody className="divide-y divide-border">
                    {paginated.map((user) => {
                      ...
                      return (
                        <tr
                          key={user.id}
                          className={`hover:bg-muted/30 transition-colors ${isBanned ? "bg-rose-500/[0.03]" : ""}`}
                        >
                              ...
                              <div>
                                <div className="font-semibold text-foreground flex items-center gap-1.5">
                                  ...
                                  {isBanned && (
                                    ...
                                    >
                                      Banned (
                                      {bannedRecord?.type === "client_request" ? "Req" : "Force"})
                                    </span>
                                  )}
                                </div>
                          ...
                          <td className="px-6 py-4 text-xs">
                            <span
                              className={`px-2.5 py-1 rounded-full font-bold border flex items-center gap-1.5 w-fit ${
                                isBanned
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            {isOwner ? (
                              ...
                              >
                                <Shield size={12} />
                                {userReferrer
                                  ? `${userReferrer} (Ref: ${getAdminReferralCode(userReferrer)})`
                                  : "OWNER"}
                              </button>
                            ) : (
                          ...
                          <td className="px-6 py-4">
                            <div className="text-sm font-bold text-foreground font-mono">
                              $
                              {(user.total_value ?? user.balance ?? 0).toLocaleString("en-US", {
                            ...
                            <div className="text-[10px] text-muted-foreground font-bold flex gap-1 items-center">
                              ...
                              <span className="text-indigo-500">
                                F: ${(user.futures_balance ?? 0).toFixed(0)}
                              </span>
                              <span className="text-warning">
                                E: ${(user.staked_balance ?? 0).toFixed(0)}
                              </span>
                            </div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="flex flex-col gap-1 items-start">
                              <span
                                className={`px-2.5 py-1 rounded-full text-xs font-medium border ${kycBadge(user.kyc_status)}`}
                              >
            ...
            <div className="px-6 py-4 border-t border-border bg-muted/30 flex items-center justify-between">
              <span className="text-sm text-muted-foreground">
                ...
                {Math.min(currentPage * PER_PAGE, processed.length)} of {processed.length}
              </span>
              <div className="flex gap-2">
                <button
                  onClick={() => setCurrentPage((p) => Math.max(1, p - 1))}
                  disabled={currentPage === 1}
                  className="px-3 py-1 text-sm border border-border rounded-md bg-card text-foreground hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Previous
                </button>
                <button
      ...
      {selectedUserForNotice && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm">
          <form
            ...
            className="bg-card w-full max-w-sm rounded-[24px] p-6 shadow-2xl relative border border-border"
          >
            <button
              type="button"
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  To: {selectedUserForNotice.username}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 mb-6 flex-shrink-0">
              ...
              <div>
                <h3 className="text-lg font-bold text-foreground">
                  Operational Footprints & IP Auditing
                </h3>
                <p className="text-xs text-muted-foreground">
                  User:{" "}
                  <span className="font-semibold text-foreground">
                    {selectedUserForLogs.username || selectedUserForLogs.email}
                  </span>
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 p-3 bg-muted/40 rounded-2xl border border-border/60 text-xs mb-4 flex-shrink-0">
              <div>
                ...
                <span className="font-mono font-bold text-foreground flex items-center gap-1">
                  ...
                  {
                    getStableSessionData(
                      selectedUserForLogs.id,
                      selectedUserForLogs.email,
                      selectedUserForLogs.username,
                    ).ip
                  }
                </span>
              </div>
                      ...
                      <div className="flex justify-between items-start gap-4">
                        <div>
                          <h4 className="text-xs font-bold text-foreground flex items-center gap-1.5 leading-snug">
                            {log.event}
                            <span
                              className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-wide tracking-wider ${
                                log.type === "auth"
      ...
      {selectedUserForAdminAssign && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <form
            onSubmit={handleAssignUserAdmin}
            className="bg-card w-full max-w-md rounded-[28px] p-6 shadow-2xl relative border border-border animate-scale-in"
          >
            <button
              type="button"
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Map client to specific administrator management panel
                </p>
              </div>
            </div>
              ...
              <div className="font-mono text-xs space-y-1">
                <div className="flex justify-between py-1">
                  ...
                  <span className="font-bold text-foreground">
                    {selectedUserForAdminAssign.username || "User"}
                  </span>
                </div>
                <div className="flex justify-between py-1">
      ...
      {selectedUserForBan && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <form
            ...
            className="bg-card w-full max-w-md rounded-[28px] p-6 shadow-2xl relative border border-warning/20 animate-scale-in"
          >
            <button
              type="button"
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Restrict trading, withdrawals, and account access
                </p>
              </div>
            </div>
            ...
            <div className="p-4 bg-muted/40 border border-border/80 rounded-2xl mb-5 text-sm space-y-3">
              <div className="font-mono text-xs space-y-1">
                <div className="flex justify-between py-1">
                  <span className="text-muted-foreground">Client:</span>
                  <span className="font-bold text-foreground">
                    {selectedUserForBan.username || "User"}
                  </span>
                </div>
                <div className="flex justify-between py-1">
                  <span className="text-muted-foreground">Email:</span>
                  <span className="font-bold text-foreground">
                    {selectedUserForBan.email || "N/A"}
                  </span>
                </div>
                <div className="flex justify-between py-1">
                  <span className="text-muted-foreground">User FTID:</span>
                  <span className="font-bold text-foreground">
                    {selectedUserForBan.ftid || "—"}
                  </span>
                </div>
              </div>
              ...
              <div className="pt-1">
                ...
                >
                  <option value="Client voluntary account suspension request">
                    Client voluntary account suspension request
                  </option>
                  <option value="Temporary self-exclusion request from user">
                    Temporary self-exclusion request from user
                  </option>
                  <option value="Terms of Service & Platform Misuse Violation">
                    Terms of Service & Platform Misuse Violation
                  </option>
                  <option value="Suspicious Trading Activity / Arbitrage Risk">
                    Suspicious Trading Activity / Arbitrage Risk
                  </option>
                  <option value="KYC Document Discrepancy & AML Flag">
                    KYC Document Discrepancy & AML Flag
                  </option>
                  <option value="Account Security Quarantine & Protection">
                    Account Security Quarantine & Protection
                  </option>
          ...
          <div className="bg-card w-full max-w-md rounded-[28px] p-6 shadow-2xl relative border border-emerald-500/20 animate-scale-in">
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Lift account suspension and restore full trading privileges
                </p>
              </div>
            </div>

            {(() => {
              const bRec = getBannedUserRecord(
                selectedUserForUnban.email || selectedUserForUnban.id,
              );
              return (
                <div className="p-4 bg-muted/40 border border-border/80 rounded-2xl mb-6 text-sm space-y-3">
                  <div className="font-mono text-xs space-y-1">
                    <div className="flex justify-between py-1">
                      <span className="text-muted-foreground">Client:</span>
                      <span className="font-bold text-foreground">
                        {selectedUserForUnban.username || "User"}
                      </span>
                    </div>
                    <div className="flex justify-between py-1">
                      <span className="text-muted-foreground">Email:</span>
                      <span className="font-bold text-foreground">
                        {selectedUserForUnban.email || "N/A"}
                      </span>
                    </div>
                    {bRec && (
                      <>
                        <div className="flex justify-between py-1 border-t border-border/40 pt-2">
                          ...
                          <span className="font-bold text-warning capitalize">
                            {bRec.type === "client_request" ? "Client Request" : "Forced"}
                          </span>
                        </div>
                        <div className="py-1">
                  ...
                  <p className="text-xs text-emerald-400/90 pt-2 border-t border-border/40">
                    Unbanning will immediately restore full login access, wallet deposits,
                    withdrawals, and spot/futures trading operations.
                  </p>
                </div>
              );
            })()}
          ...
          <div className="bg-card w-full max-w-md rounded-[28px] p-6 shadow-2xl relative border border-destructive/20 animate-scale-in">
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Permanent erasure of user data and credentials
                </p>
              </div>
            </div>
            ...
            <div className="p-4 bg-danger/5 border border-danger/15 rounded-2xl mb-6 text-sm text-foreground/90 space-y-3">
              <div className="font-mono text-xs bg-muted/60 p-3 rounded-xl border border-border space-y-1">
                <div className="flex justify-between py-0.5">
                  ...
                  <span className="font-bold text-foreground">
                    {selectedUserForDelete.username || "N/A"}
                  </span>
                </div>
                <div className="flex justify-between py-0.5">
                  <span className="text-muted-foreground">Email:</span>
                  <span className="font-bold text-foreground">
                    {selectedUserForDelete.email || "N/A"}
                  </span>
                </div>
              ...
              <div className="p-2.5 bg-danger/10 border border-danger/20 rounded-xl text-xs text-danger/90 leading-relaxed flex items-start gap-2">
                ...
                <span>
                  Permanent action: This purges user assets, ledger deposits, withdrawals, open
                  futures positions, and auth profile records.
                </span>
              </div>
            </div>
          ...
          <div className="bg-card w-full max-w-md rounded-[28px] p-6 shadow-2xl relative border border-rose-500/20 animate-scale-in">
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Clean up all registered e2e test users
                </p>
              </div>
            </div>
            ...
            <div className="p-4 bg-rose-500/5 border border-rose-500/15 rounded-2xl mb-6 text-sm text-foreground/90 space-y-3">
              <p className="text-xs font-semibold text-foreground">
                Found <span className="text-rose-400 font-bold">{e2eAccounts.length}</span>{" "}
                account(s) starting with "e2e":
              </p>
              <div className="max-h-36 overflow-y-auto space-y-1 font-mono text-xs bg-muted/60 p-2.5 rounded-xl border border-border custom-scrollbar">
                {e2eAccounts.map((u) => (
                  <div
                    key={u.id}
                    className="flex justify-between py-0.5 text-[11px] border-b border-border/30 last:border-0"
                  >
                    <span className="truncate max-w-[180px] font-bold text-foreground">
                      {u.email || u.username}
                    </span>
                    <span className="text-muted-foreground opacity-75">
                      {u.ftid || u.id.substring(0, 8)}
                    </span>
                  </div>
                ))}
              </div>
              ...
              <div className="p-2.5 bg-rose-500/10 border border-rose-500/20 rounded-xl text-xs text-rose-400/90 leading-relaxed flex items-start gap-2">
                ...
                <span>
                  This will permanently delete all {e2eAccounts.length} test accounts, their
                  portfolios, ledger entries, and Supabase auth records.
                </span>
              </div>
            </div>
