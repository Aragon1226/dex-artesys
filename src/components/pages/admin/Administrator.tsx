...
const ADMIN_PAGES: { key: keyof PagePermissions; label: string; desc: string }[] = [
  ...
  {
    key: "sample-tokens",
    label: "Sample Token Control",
    desc: "Manage price trends, factors & percentages for sample tokens",
  },
...
const AdminAdministrator = () => {
  ...
  const currentAccount = customAccountsList.find(
    ...
    return (
      <div className="p-8 min-h-[80vh] flex flex-col items-center justify-center text-center">
        ...
        <p className="text-muted-foreground max-w-md mb-8">
          This portal is restricted to platform Owners and Administrators. Staff members are not
          authorized to create or manage other staff accounts.
        </p>
        <button
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div
        id="group-referral-tracker"
        className="bg-card border border-border p-6 rounded-[32px] shadow-lg space-y-6"
      >
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-border/50 pb-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-black uppercase tracking-tight">
                Group Referral Tracker
              </h2>
            </div>
            <p className="text-xs font-semibold text-muted-foreground">
              Monitor group registration mapping. Securely assign new registrations under designated
          ...
          {isOwner && (
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                <label className="text-xs font-black uppercase tracking-wider text-muted-foreground whitespace-nowrap">
                  Filter Group:
                </label>
                <select
          ...
          <div className="lg:col-span-1 bg-muted/20 border border-border/80 p-5 rounded-2xl flex flex-col justify-between space-y-4">
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground font-semibold leading-relaxed">
                Provide your short referral link or code (
                <strong className="text-primary font-mono">
                  {getReferralCodeForCurrentUser(currentUserEmail)}
                </strong>
                ) to clients. Any account created using this link will automatically map directly
                under your group panel for seamless management, deposit/withdrawal verification, and
                client support.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  value={
                    currentUserEmail?.toLowerCase() === "admin2@crypxpro.com"
                      ? `${window.location.origin.includes("artesys.com") ? "https://artesys.com" : window.location.origin.replace("//admin.", "//")}/auth`
                      : `${window.location.origin.includes("artesys.com") ? "https://artesys.com" : window.location.origin.replace("//admin.", "//")}/auth?ref=${getReferralCodeForCurrentUser(currentUserEmail)}`
                  }
                ...
                <button
                  ...
                  onClick={() => {
                    const baseUrl = window.location.origin.includes("artesys.com")
                      ? "https://artesys.com"
                      : window.location.origin.replace("//admin.", "//");
                    const isAdmin2 = currentUserEmail?.toLowerCase() === "admin2@crypxpro.com";
                    const refLink = isAdmin2
                      ? `${baseUrl}/auth`
                      : `${baseUrl}/auth?ref=${getReferralCodeForCurrentUser(currentUserEmail)}`;
                    navigator.clipboard.writeText(refLink);
                    setCopied(true);
                    toast.success(
                      isAdmin2
                        ? "Global referral link copied!"
                        : `Referral link copied! (Code: ${getReferralCodeForCurrentUser(currentUserEmail)})`,
                    );
                ...
                <div className="w-full space-y-1.5">
                  ...
                  {referrals
                    ...
                    .map((refItem, idx) => (
                      <div
                        key={idx}
                        className="grid grid-cols-12 text-xs font-bold bg-card border border-border/60 p-2.5 rounded-xl hover:border-primary/10 transition-all items-center"
                      >
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Map client to specific administrator management group
                </p>
              </div>
            </div>
