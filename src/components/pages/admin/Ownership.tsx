...
const ADMIN_PAGES: { key: keyof PagePermissions; label: string; desc: string }[] = [
  ...
  { key: "futures", label: "Futures Control", desc: "Control active futures and win rates" },
  {
    key: "sample-tokens",
    label: "Spot Control",
    desc: "Control spot token prices and trend schedules",
  },
    ...
    return (
      <div className="p-8 min-h-[80vh] flex flex-col items-center justify-center text-center">
        ...
        <p className="text-muted-foreground max-w-md mb-8">
          This panel is reserved exclusively for the platform Owner. Unauthorized personnel are
          strictly forbidden from viewing ownership controls.
        </p>
        <button
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-card border border-border p-6 rounded-[32px] shadow-lg">
        <div className="space-y-1">
          ...
          <p className="text-xs font-semibold text-muted-foreground">
            Platform Owner Core. Create secondary Administrator accounts with custom permissions and
            manage credentials.
          </p>
        </div>
      ...
      <div className="flex items-start gap-4 p-5 bg-warning/10 border border-warning/20 rounded-[24px] text-warning">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs opacity-90 leading-relaxed font-medium">
            Your primary owner account (
            <span className="underline font-bold">{currentUserEmail}</span>) has permanent, absolute
            access across all panels. Created administrator accounts listed below will be restricted
            strictly to their assigned viewing privileges.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-1 bg-card border border-border rounded-[32px] p-6 shadow-xl space-y-6 h-fit">
          <div className="space-y-1 border-b border-border pb-4">
            ...
            <p className="text-xs text-muted-foreground">
              Register a new secure admin account with targeted rights.
            </p>
          </div>
        ...
        <div className="xl:col-span-2 bg-card border border-border rounded-[32px] p-6 shadow-xl space-y-6 flex flex-col">
          <div className="space-y-1 border-b border-border pb-4">
            ...
            <p className="text-xs text-muted-foreground">
              Manage accounts, change credentials, and edit page permissions instantly.
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center py-16 text-center text-muted-foreground">
              ...
              <p className="text-xs text-muted-foreground max-w-sm">
                Use the registration form on the left to add your first secure Administrator
                account.
              </p>
            </div>
          ) : (
            <div className="space-y-6 flex-1 overflow-y-auto max-h-[70vh] pr-2">
              {accounts.map((admin) => (
                <div
                  key={admin.id}
                  className="p-5 bg-muted/20 border border-border rounded-2xl space-y-4 hover:border-primary/20 transition-all"
                >
                      ...
                      {editingAccountId === admin.id ? (
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-mono bg-card px-2.5 py-1 rounded-lg border border-border text-muted-foreground/80">
                            Pass: <span className="font-black font-sans">•••••</span> (
                            {admin.password})
                          </span>
                          <button
                            onClick={() => {
            ...
            <div className="space-y-3">
              <p className="text-xs text-muted-foreground leading-relaxed font-semibold">
                ...
                <span className="font-bold text-foreground font-mono bg-muted px-1.5 py-0.5 rounded">
                  {adminToDelete.customId}
                </span>{" "}
                (<span className="font-bold text-foreground underline">{adminToDelete.email}</span>
                ). This will revoke their access to all admin panels and remove their credentials
                immediately.
              </p>
            </div>
