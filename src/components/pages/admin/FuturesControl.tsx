...
const FuturesControl = () => {
  ...
  const handleSetOutcome = async (
    ...
    if (
      targetUser &&
      targetUser.force_win === updates.force_win &&
      targetUser.force_loss === updates.force_loss
    )
      return;
    ...
    try {
      ...
      const outcomeText =
        outcome === "win" ? "FORCE_WIN" : outcome === "loss" ? "FORCE_LOSS" : "NORMAL_CALCULATION";
      ...
      recordActivityLog({
        ...
        title:
          outcome === "normal"
            ? "Reset Futures Market Override"
            : `Set Futures Outcome to ${outcomeText}`,
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage forced outcomes and risk settings.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <div className="bg-card border border-border rounded-lg p-1 flex items-center">
            ...
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${showForcedOnly ? "translate-x-6" : "translate-x-1"}`}
              />
            </button>
          </div>
        </div>
              ...
              <table className="w-full text-left min-w-[600px]">
                ...
                <tbody className="divide-y divide-border">
                  {paginated.map((user) => (
                    <tr
                      key={user.id}
                      className={`hover:bg-muted/30 transition-colors ${user.force_win ? "bg-primary/5" : ""}`}
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div
                            className={`h-10 w-10 rounded-full flex items-center justify-center font-bold ${user.force_win && !user.force_loss ? "bg-success/10 text-success" : user.force_win && user.force_loss ? "bg-danger/10 text-danger" : "bg-primary/10 text-primary"}`}
                          >
                      ...
                      <td className="px-6 py-4 text-foreground font-mono">
                        $
                        {(user.futures_balance ?? 0).toLocaleString(undefined, {
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center gap-3">
                          ...
                          <div className="flex flex-col items-center gap-1 group">
                            ...
                            >
                              <span
                                className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${user.force_win ? "translate-x-5" : "translate-x-1"}`}
                              />
                            </button>
            ...
            <div className="px-6 py-4 border-t border-border bg-muted/30 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3 py-1 text-sm border border-border rounded-md bg-card text-foreground hover:bg-muted disabled:opacity-50"
                >
                  Previous
                </button>
                <button
