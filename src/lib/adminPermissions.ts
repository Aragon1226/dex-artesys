...
export const PRIMARY_OWNERS = ["arkarnaung009@gmail.com"];
...
export function saveUserPermission(config: UserPermissionConfig) {
  ...
  const customAccounts = getCustomAccounts();
  const accountIdx = customAccounts.findIndex(
    (a) => a.email.toLowerCase().trim() === normalizedEmail,
  );
  if (accountIdx !== -1) {
  ...
  try {
    ...
    if (data) {
      ...
      if (data.admin_permissions) {
        try {
          permissions =
            typeof data.admin_permissions === "string"
...
export function getAdminReferralCode(
  adminIdOrAccountOrEmail: string | CustomAccount | null | undefined,
): string {
...
export function isTestOrE2EAccount(
  item: string | { email?: string | null; username?: string | null } | null | undefined,
): boolean {
...
export function getUserReferrals(): UserReferral[] {
  ...
  DEFAULT_USER_REFERRALS.forEach((def) => {
    ...
    if (!isTestOrE2EAccount(normDefEmail)) {
      const existingIdx = referrals.findIndex(
        (r) => r.userEmail.toLowerCase().trim() === normDefEmail,
      );
      if (existingIdx === -1) {
        referrals.push(def);
      }
...
export function setReferrerForUser(
  email: string,
  ...
  const index = referrals.findIndex(
    (r) => r.userEmail.toLowerCase().trim() === normEmail || (userId && r.userId === userId),
  );
...
export function filterUsersByAdminGroup<
  T extends { id?: string; user_id?: string; email?: string | null; username?: string | null },
>(items: T[], currentAdminId: string | null): T[] {
  ...
  return nonTestItems.filter((item) => {
    ...
    return (
      normReferrer === normCurrentAdmin ||
      referrer.toLowerCase().trim() === currentAdminId.toLowerCase().trim()
    );
  });
}
...
export async function syncCustomAccountsWithSupabase(): Promise<CustomAccount[]> {
  try {
    ...
    if (error) {
      console.warn(
        "Could not load custom accounts from real Supabase, using local fallback.",
        error,
      );
      return getCustomAccounts();
    }
    ...
    if (data && data.length > 0) {
      ...
      dbAccounts.forEach((dbAcc) => {
        const idx = merged.findIndex(
          (l) => l.email.toLowerCase().trim() === dbAcc.email.toLowerCase().trim(),
        );
        if (idx !== -1) {
          merged[idx] = dbAcc;
        } else {
...
export async function saveCustomAccountToSupabase(account: CustomAccount): Promise<void> {
  try {
    ...
    try {
      ...
      console.warn(
        "Failed to create custom admin server-side. Fallback to direct upsert.",
        rpcError,
      );
...
export async function deleteCustomAccountFromSupabase(email: string): Promise<void> {
  try {
    ...
    try {
      ...
      console.warn(
        "Failed to delete custom admin server-side. Fallback to direct delete.",
        rpcError,
      );
...
export async function syncUserReferralsWithSupabase(): Promise<UserReferral[]> {
  try {
    ...
    dbReferrals.forEach((dbRef) => {
      const idx = merged.findIndex(
        (l) => l.userEmail.toLowerCase().trim() === dbRef.userEmail.toLowerCase().trim(),
      );
      if (idx !== -1) {
        merged[idx] = dbRef;
      } else {
    ...
    for (const def of DEFAULT_USER_REFERRALS) {
      const inDb = dbReferrals.some(
        (d) => d.userEmail.toLowerCase().trim() === def.userEmail.toLowerCase().trim(),
      );
      if (!inDb) {
        saveUserReferralToSupabase(def).catch((e) =>
          console.warn("Auto-sync default referral error:", e),
        );
      }
    }
...
export async function saveUserReferralToSupabase(referral: UserReferral): Promise<void> {
  try {
    ...
    const { error } = await supabase
      .from("user_referrals")
      .upsert(dbPayload, { onConflict: "user_email" });
    if (error) {
...
export async function syncAdminWalletsWithSupabase(): Promise<AdminWalletConfig[]> {
  try {
    ...
    if (error) {
      console.warn(
        "Could not load admin wallet configs from Supabase, using local fallback.",
        error,
      );
...
export async function saveAdminWalletToSupabase(wallet: AdminWalletConfig): Promise<void> {
  try {
    ...
    const { error } = await supabase
      ...
      .upsert(dbPayload, { onConflict: "admin_id,symbol,network" });
    if (error) {
...
export async function deleteAdminWalletFromSupabase(
  adminId: string,
  symbol: string,
  network: string,
): Promise<void> {
    ...
    if (record.userId) {
      await supabase
        ...
        .insert({
          ...
          title:
            record.type === "client_request"
              ? "Account Suspension Processed"
...
export async function unbanUserRecord(emailOrId: string): Promise<void> {
  ...
  const updatedList = list.filter(
    (u) => u.email.toLowerCase().trim() !== clean && u.userId !== clean,
  );
  saveBannedUsers(updatedList);
}
